...
function PurchaseOrder() {
  ...
  const fieldEdits = form.values.editDetails.map((_, index) => (
    <tr key={index}>
      ...
      <td>
        {form.values.editDetails[index].value != "" &&
        typeof form.values.editDetails[index].value != "undefined" ? (
          <>
            {variables.editPOList.length > 1 ? (
              <ActionIcon
                color="red"
                mr={20}
                variant="hover"
                onClick={() =>
                  openConfirmModal2(form.values.editDetails[index].value)
                }
              >
                <Trash size={16} />
              </ActionIcon>
            ) : (
              <ActionIcon disabled color="red" mr={20} variant="hover">
                <Trash size={16} />
              </ActionIcon>
            )}
          </>
          ...
          <ActionIcon
            onClick={() => form.removeListItem("editDetails", index)}
            color="red"
            mr={20}
            variant="hover"
          >
  ...
  const handleAdd = async (e) => {
    ...
    const response = await handleAddPO(e);
  ...
  const handleEdits = async (e) => {
    ...
    const response = await handleEditPO(e);
          ...
          <form onSubmit={form.onSubmit((values) => handleEdits(values))}>
            ...
            <Group position="right" mt={10}>
              ...
              <Button
                ...
                mb="sm"
                onClick={() =>
                  form.insertListItem("editDetails", {
                    brand_id: "",
                    group_id: "",
                    subgroup_id: "",
                    product_id: "",
            ...
            <Group position="right" mt="md">
              {form.values.editDetails.length >= 1 ? (
                <Button type="submit" color="zevcore">
                  Submit
                </Button>
              ) : (
                <Button disabled color="zevcore">
                  Submit
                </Button>
              )}
            </Group>
