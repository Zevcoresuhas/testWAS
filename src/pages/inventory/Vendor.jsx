...
function Vendor() {
  ...
  const form = useForm({
    ...
    validate: {
      name: (value) => (value.length < 1 ? "Vendor name is required" : null),
    },
  });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      if (mounted) {
        //   For get all the vendor data list
        const response = await handleGetVendor();
  ...
  const openConfirmModal = (e) => {
    ...
    modals.openConfirmModal({
      title: "Do you want to delete this vendor value",
  ...
  const handleConfirmDelete = async (e) => {
    const response = await handleDeleteVendor(e);
  ...
  const handleEdit = (e) => {
    ...
    formEdit.setFieldValue("phone_number2", datas.phone_number2);
  ...
  // Edit vendor data compoennt
  const EditVendor = async (e) => {
    setVariables({ ...variables, submitLoading: true });
    const response = await handleEditVendor(e);
  ...
  const AddVendor = async (e) => {
    setVariables({ ...variables, submitLoading: true });
    const response = await handleAddVendor(e);
  ...
  // Upload bulk json data vendor
  const BulkDataUpload = async () => {
    setVariables({ ...variables, submitLoading: true });

    const response = await handleBulkVendor(bulkData);
    // Check for response for actions
    if (response.status === 200) {
      interval.stop();
      notificationHelper({
        color: "green",
        title: "Success",
        message: "Bulk vendor added successfully",
      });
      form.reset();
      setVariables({
        ...variables,
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <div>
                <Group spacing="xs">
                  ...
                  <CSVLink
                    data={variables.data}
                    headers={[{ label: " Vendor Name", key: "label" }]}
                    filename="vendor.csv"
                  ...
                  {/* Drawer open for adding new vendor data */}
                  <Button
                    variant="outline"
                    color="zevcore"
                    size="xs"
        ...
        <div className="zc-p-3">
          ...
          >
            <div className="zc-pr-3 zc-pl-3">
              {/* Vendor adding form name */}
              <form onSubmit={form.onSubmit((values) => AddVendor(values))}>
                <TextInput
                  variant="filled"
                ...
                <TextInput
                  variant="filled"
                  value={form.values.phone_number2}
                  label="Alternate Phone Number"
                  placeholder="Alternate Phone Number"
                  {...form.getInputProps("phone_number2")}
                />
                <TextInput
                  variant="filled"
        ...
        <div className="zc-p-3">
          ...
          >
            <div className="zc-pr-3 zc-pl-3">
              {/* Vendor adding form name */}
              <form
                onSubmit={formEdit.onSubmit((values) => EditVendor(values))}
              >
                <TextInput
                  variant="filled"
                ...
                <TextInput
                  variant="filled"
                  value={formEdit.values.phone_number2}
                  label="Alternate Phone Number"
                  placeholder="Alternate Phone Number"
                  {...formEdit.getInputProps("phone_number2")}
                />
                <TextInput
                  variant="filled"
      ...
      >
        ...
        <div className="zc-p-3">
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            ...
            <div
              onClick={() => onDownload({ data: "vendor" })}
              style={{
