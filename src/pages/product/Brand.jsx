...
import { terminologies, dataSlice, onDownload } from "../../helpers/common"; // Common fuctions uses for applications
...
function Brand() {
  ...
  const formEdit = useForm({
    ...
    validate: {
      editName: (value) => (value.length < 1 ? "Brand name is required" : null),
    },
  });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      if (mounted) {
        ...
        const response = await handleGetBrand();
  ...
  const openConfirmModal = (e) => {
    ...
    modals.openConfirmModal({
      title: "Do you want to delete this brand value",
  ...
  const handleConfirmDelete = async (e) => {
    const response = await handleDeleteBrand(e);
  ...
  const EditBrand = async (e) => {
    ...
    const response = await handleEditBrand(e);
  ...
  const AddBrand = async (e) => {
    setVariables({ ...variables, submitLoading: true });
    const response = await handleAddBrand(e);
  ...
  const BulkDataUpload = async () => {
    ...
    const response = await handleBulkBrand(bulkData);
    // Check for response for actions
    if (response.status === 200) {
      interval.stop();
      notificationHelper({
    ...
    <div>
      ...
      >
        <div>
          ...
          <div className="zc-p-3">
            ...
            <form onSubmit={form.onSubmit((values) => AddBrand(values))}>
              <TextInput
                required
                variant="filled"
      ...
      >
        <div>
          ...
          <div className="zc-p-3">
            {/* Brand adding form name */}
            <form onSubmit={formEdit.onSubmit((values) => EditBrand(values))}>
              <TextInput
                required
                variant="filled"
