...
function Cash() {
  ...
  const rows = sortedData.map((row, index) => (
    <tr key={row.label}>
      ...
      <td>{row.transaction_type == "Income" ? row.amount : ""}</td>
      <td>{row.transaction_type == "Expense" ? row.amount : ""}</td>
  ...
  const handleConfirmDelete = async (e) => {
    const response = await handleDeleteCash(e);
  ...
  const EditCash = async (e) => {
    ...
    const response = await handleEditCash(e);
  ...
  const AddCash = async (e) => {
    setVariables({ ...variables, submitLoading: true });
    const response = await handleAddCash(e);
      ...
      <Skeleton radius="md" visible={variables.skeletonLoading}>
        <Card className="border">
          <Tabs
            ...
            variant="outline"
            defaultValue="cash"
            onTabChange={(value) => navigate(`/bank_cash`)}
          >
            <Tabs.List>
