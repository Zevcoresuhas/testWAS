...
import { handleGetEstimate, handleGetInvoice } from "../../helpers/apis"; // Import for axios requests list for this pages
...
function Estimate() {
  ...
  const form = useForm({
    ...
    validate: {
      name: (value) => (value.length < 1 ? "Estimate name is required" : null),
    },
  });
  ...
  const formEdit = useForm({
    ...
    validate: {
      editName: (value) =>
        value.length < 1 ? "Estimate name is required" : null,
    },
  });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      if (mounted) {
        ...
        const response = await handleGetEstimate();
  ...
  const rows = sortedData.map((row, index) => (
    <tr key={row.label}>
      ...
      <td>{row.total}</td>
      <td>{row.paid}</td>
      <td>{new Date(row.estiamte_date).toLocaleDateString()}</td>
      ...
      <td justifycontent="right" align="right">
        <Menu shadow="sm" size="xs">
          ...
          <Menu.Dropdown>
            <Menu.Item
              onClick={() => navigate("/print_estimate/" + row.estimate_number)}
              icon={<Eye size={14} />}
            >
              View
            </Menu.Item>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <div>
                <Group spacing="xs">
                  ...
                  <CSVLink
                    data={variables.data}
                    headers={[{ label: " Estimate Name", key: "label" }]}
                    filename="sale.csv"
